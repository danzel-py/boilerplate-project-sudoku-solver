...
function validate(puzzleString) { // RETURNS BOOLEAN FALSE IF INVALID
    let regex = /[^0-9.]/
    if (regex.test(puzzleString) || puzzleString.length !== 81) return false
    return true
}

function checkRowPlacement(puzzleString, row, column, value) { // RETURNS BOOLEAN: FALSE IF INVALID
    let result = true
    let rowIndex = row - 1
...
function checkColPlacement(puzzleString, row, column, value) { // RETURNS BOOLEAN: FALSE IF INVALID
    let result = true
    let rowIndex = row - 1
    let columnIndex = column - 1
    value = value.toString()
...
function checkRegionPlacement(puzzleString, row, column, value) { // RETURNS BOOLEAN: FALSE IF INVALID
    let result = true
    let rowIndex = row - 1
    let columnIndex = column - 1
    value = value.toString()
...
function solve(puzzleString) {
    if (!validate(puzzleString)) return 'dumb puzzle'
    ...
    console.log('stringSolved')
    
    return solve(stringSolved)

}

let bro = solve('..9..5.1.85.4....2432......1...69.83.9.....6.62.71...9......1945....4.37.4.3..6..')
bro